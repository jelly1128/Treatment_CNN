...
def test(config: dict, test_data_dirs: list, fold_idx: int, window_size: int):
    logging.info(f"Starting test for fold {fold_idx+1} with window size {window_size}...")
    # config.paths.save_dir = f'/home/tanaka/0218/Treatment_CNN/{config.test.num_classes}class_resnet18_test/{config.test.num_classes}class_resnet18_test' + '_fold' + str(fold_idx+1)
    config.paths.save_dir = f'/home/tanaka/0218/Treatment_CNN/{config.test.num_classes}class_resnet18_test/' + 'fold' + str(fold_idx+1)
    # save_dirを作成
    Path(config.paths.save_dir).mkdir(exist_ok=True)
    # config.paths.model = f'/home/tanaka/0218/Treatment_CNN/{config.test.num_classes}class_resnet18/fold_' + str(fold_idx+1) + '/best_model.pth'
    config.paths.model = f'/home/tanaka/0218/Treatment_CNN/multilabel_cross_valid_{config.test.num_classes}class/fold' + str(fold_idx+1) + '/best_model.pth'
    ...
    converter.save_hard_multilabel_results(hard_multilabel_results, Path(config.paths.save_dir), methods = 'threshold_50%')
    # 正解ラベルの可視化
    ...
    save_video_metrics_to_csv(video_metrics, Path(config.paths.save_dir), methods = 'threshold_50%')
    save_overall_metrics_to_csv(overall_metrics, Path(config.paths.save_dir), methods = 'threshold_50%')
    
    # ハードラベルの結果を読み込む
    hard_multilabel_results = {}
    for folder_name in test_data_dirs:
        csv_path = Path(f'{config.paths.save_dir}/{folder_name}/threshold_50%/threshold_50%_results_{folder_name}.csv')
    ...
    sliding_window_dir = Path(config.paths.save_dir) / f'sliding_window_{window_size}_results'
...
def main():
    ...
    for window_size in window_sizes:
        ...
        for fold_idx, fold in enumerate(split_folders):
            ...
            sliding_window_results = test(config, test_data_dirs, fold_idx, window_size)
